...
type CryptoComData = {
  b?: string; k?: string; bv?: string; kv?: string
}
type CryptoComResponse = {
  result?: { data?: CryptoComData }
}

export class CryptoComAdapter extends BaseExchangeAdapter {
  config: ExchangeConfig = EXCHANGE_REGISTRY.cryptocom
  ...
  private async fetchAndEmit(symbol: string): Promise<void> {
    ...
    try {
      const res = await fetch(
        `${this.config.restUrl}/v2/public/get-ticker?instrument_name=${instrument}`,
        { signal: controller.signal }
      )
      if (!res.ok) return
      const body = await res.json() as CryptoComResponse
      const data = body.result?.data
      if (!data) return
      const bid = parseFloat(data.b ?? '0')
      const ask = parseFloat(data.k ?? '0')
      ...
      const tick: PriceTick = {
        ...
        bidSize: parseFloat(parseFloat(data.bv ?? '0').toFixed(8)),
        askSize: parseFloat(parseFloat(data.kv ?? '0').toFixed(8)),
        timestamp: Date.now(),
        source: 'rest',
      }
