...
// AscendEX uses BTC/USDT (slash already matches our format)

type AscendExData = {
  ask?: string[]; bid?: string[]; close?: string
}
type AscendExResponse = { code?: number; data?: AscendExData }

export class AscendExAdapter extends BaseExchangeAdapter {
  config: ExchangeConfig = EXCHANGE_REGISTRY.ascendex
  ...
  private async fetchAndEmit(symbol: string): Promise<void> {
    ...
    try {
      const res = await fetch(
        `${this.config.restUrl}/api/pro/v1/spot/ticker?symbol=${encodeURIComponent(symbol)}`,
        { signal: controller.signal }
      )
      if (!res.ok) return
      const body = await res.json() as AscendExResponse
      if (body.code !== 0 || !body.data) return
      const data = body.data
      // ask/bid are [price, size] arrays
      ...
      const tick: PriceTick = {
        ...
        bidSize: parseFloat(parseFloat(data.bid?.[1] ?? '0').toFixed(8)),
        askSize: parseFloat(parseFloat(data.ask?.[1] ?? '0').toFixed(8)),
        timestamp: Date.now(),
        source: 'rest',
      }
