...
const SYMBOLS = [
  ...
  'NEAR/USDT', 'UNI/USDT', 'ATOM/USDT', 'ARB/USDT', 'OP/USDT',
  'SUI/USDT', 'INJ/USDT', 'PEPE/USDT', 'SHIB/USDT',
]

function toBtseSymbol(sym: string): string {
  return sym.replace('/', '-')
}

type BtseSummary = {
  symbol?: string; highestBid?: number; lowestAsk?: number; last?: number
}

export class BtseAdapter extends BaseExchangeAdapter {
  config: ExchangeConfig = EXCHANGE_REGISTRY.btse
  ...
  private async fetchAndEmit(symbol: string): Promise<void> {
    const btseSym = toBtseSymbol(symbol)
    const controller = new AbortController()
    const t = setTimeout(() => controller.abort(), 5_000)
    try {
      const res = await fetch(
        `${this.config.restUrl}/spot/api/v3.2/market_summary?symbol=${btseSym}`,
        { signal: controller.signal }
      )
      if (!res.ok) return
      const data = await res.json() as BtseSummary[]
      if (!Array.isArray(data) || !data.length) return
      const item = data[0]
      const bid = item.highestBid ?? item.last ?? 0
      const ask = item.lowestAsk ?? item.last ?? 0
