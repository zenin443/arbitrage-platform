import { BaseExchangeAdapter, ExchangeConfig, PriceTick, NetworkStatus } from './base'
import { EXCHANGE_REGISTRY } from '../../registry/exchangeRegistry'
import { SYMBOLS } from '../../config/symbols'

const BATCH_SIZE = 25

function toPhemexSymbol(sym: string): string {
  return sym.replace('/', '')
}

type PhemexTickerResult = {
  bidPrice?: string; askPrice?: string
  bidEp?: number; askEp?: number
}
type PhemexResponse = { code?: number; data?: PhemexTickerResult; result?: PhemexTickerResult }

function parsePhemexPrice(raw: string | number | undefined, scaleFactor = 1): number {
  if (raw === undefined || raw === null) return 0
  const n = typeof raw === 'string' ? parseFloat(raw) : raw
  if (isNaN(n)) return 0
  // Extended price values (Ep) are very large integers; divide by scale factor
  return n > 100_000_000 ? n / 10_000 : n * scaleFactor
}

export class PhemexAdapter extends BaseExchangeAdapter {
  config: ExchangeConfig = EXCHANGE_REGISTRY.phemex
  private active = false
  private connected = false
  private onTick: ((tick: PriceTick) => void) | null = null
  private lastTicks = new Map<string, PriceTick>()
  private tickCount = 0
  private statusTimer: ReturnType<typeof setInterval> | null = null
  private pollCursor = 0

  async connect(onTick: (tick: PriceTick) => void): Promise<void> {
    this.onTick = onTick
    this.active = true
    this.connected = true
    this.statusTimer = setInterval(() => {
      this.log(`ticks=${this.tickCount}`)
    }, 30_000)
    void this.pollLoop()
  }

  private async pollLoop(): Promise<void> {
    let backoffMs = 2000
    while (this.active) {
      try {
        const batch = SYMBOLS.slice(this.pollCursor, this.pollCursor + BATCH_SIZE)
        await Promise.allSettled(batch.map(sym => this.fetchAndEmit(sym)))
        this.pollCursor = (this.pollCursor + BATCH_SIZE) % SYMBOLS.length
        backoffMs = 2000
        await this.delay(5_000)
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err)
        this.error(`poll error: ${msg}`)
        await this.delay(backoffMs)
        backoffMs = Math.min(backoffMs * 2, 30_000)
      }
    }
  }

  private async fetchAndEmit(symbol: string): Promise<void> {
    const phSym = toPhemexSymbol(symbol)
    const controller = new AbortController()
    const t = setTimeout(() => controller.abort(), 5_000)
    try {
      const res = await fetch(
        `${this.config.restUrl}/md/v3/ticker/24hr?symbol=${phSym}`,
        { signal: controller.signal }
      )
      if (!res.ok) return
      const body = await res.json() as PhemexResponse
      const data = body.data ?? body.result
      if (!data) return

      let bid = parseFloat(data.bidPrice ?? '0')
      let ask = parseFloat(data.askPrice ?? '0')

      // Fallback to extended price if string prices unavailable
      if (!bid && data.bidEp) bid = parsePhemexPrice(data.bidEp)
      if (!ask && data.askEp) ask = parsePhemexPrice(data.askEp)

      if (!bid || !ask || isNaN(bid) || isNaN(ask)) return
      const tick: PriceTick = {
        exchangeId: this.config.id,
        symbol,
        bid: parseFloat(bid.toFixed(8)),
        ask: parseFloat(ask.toFixed(8)),
        bidSize: 0,
        askSize: 0,
        timestamp: Date.now(),
        source: 'rest',
      }
      this.lastTicks.set(symbol, tick)
      this.onTick?.(tick)
      this.tickCount++
    } finally {
      clearTimeout(t)
    }
  }

  disconnect(): void {
    this.active = false
    this.connected = false
    if (this.statusTimer) { clearInterval(this.statusTimer); this.statusTimer = null }
    this.log('disconnected')
  }

  async fetchTicker(symbol: string): Promise<PriceTick> {
    const cached = this.lastTicks.get(symbol)
    if (cached) return cached
    await this.fetchAndEmit(symbol)
    const tick = this.lastTicks.get(symbol)
    if (!tick) throw new Error(`${this.config.id}: no data for ${symbol}`)
    return tick
  }

  async fetchNetworkStatus(_coin: string): Promise<NetworkStatus[]> { return [] }

  async fetchOrderbookDepth(
    _symbol: string, _limit?: number
  ): Promise<{ bids: [number, number][]; asks: [number, number][] }> {
    return { bids: [], asks: [] }
  }

  isConnected(): boolean { return this.connected && this.active }

  private delay(ms: number): Promise<void> {
    return new Promise(resolve => setTimeout(resolve, ms))
  }
}
